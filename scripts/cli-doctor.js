...
function checkNodeVersion(nodeVersion) {
    ...
    if (isBelowMinimum) {
        return fail(CHECK_NODE_VERSION, `Node ${nodeVersion} is below the required >= ${MIN_NODE_VERSION}. Upgrade before running build/deploy.`, { found: nodeVersion, required: `>=${MIN_NODE_VERSION}` });
    }
...
function checkPolicyExists(policyPath) {
    if (!fs.existsSync(policyPath)) {
        return fail(CHECK_POLICY_EXISTS, `Policy file not found at ${policyPath}. Run \`npx cdn-security init\` to scaffold one.`, { policyPath });
    }
...
function checkSchemaVersion(policyDoc) {
    ...
    if (version === undefined) {
        return fail(CHECK_POLICY_SCHEMA_VERSION, 'Policy has no `version` field. Add `version: 1`.');
    }
    if (version !== SCHEMA_CURRENT_VERSION) {
        return fail(CHECK_POLICY_SCHEMA_VERSION, `Policy declares version ${version} but this CLI ships schema v${SCHEMA_CURRENT_VERSION}. Run \`cdn-security migrate\` or upgrade the CLI.`, { found: version, expected: SCHEMA_CURRENT_VERSION });
    }
...
function collectReferencedEnvVars(policyDoc) {
    ...
    for (const route of routes) {
        ...
        if (!gate)
            continue;
        if (typeof gate.token_env === 'string' && gate.token_env.length > 0)
            seen.add(gate.token_env);
        if (typeof gate.credentials_env === 'string' && gate.credentials_env.length > 0)
            seen.add(gate.credentials_env);
        if (typeof gate.secret_env === 'string' && gate.secret_env.length > 0)
            seen.add(gate.secret_env);
    }
...
function checkEnvVars(policyDoc, envProvider) {
    ...
    if (missing.length > 0) {
        return fail(CHECK_ENV_VARS, `Policy references env vars that are not set: ${missing.join(', ')}. CloudFront Functions cannot read env at runtime, so these are baked into the build artifact.`, { referenced, missing });
    }
    return pass(CHECK_ENV_VARS, `All ${referenced.length} referenced env var(s) set: ${referenced.join(', ')}.`, { referenced, missing: [] });
}
...
function checkDependencies(cwd, spawnSyncImpl) {
    ...
    if (problems.length > 0) {
        return fail(CHECK_DEPENDENCIES, `npm reports dependency problems: ${problems.join('; ')}. Run \`npm install\` to resolve.`, { problems });
    }
