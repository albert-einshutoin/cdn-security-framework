...
const { runDoctor, collectReferencedEnvVars, checkNodeVersion, checkPolicyExists, checkPolicyParses, checkSchemaVersion, checkEnvVars, checkDistWritable, checkDependencies, tryParsePolicy, resolvePolicyPath, MIN_NODE_MAJOR, MIN_NODE_VERSION, SCHEMA_CURRENT_VERSION, } = require('./cli-doctor.js');
...
test('collectReferencedEnvVars: picks token_env, credentials_env, secret_env across routes', () => {
    ...
    assert.deepStrictEqual(collectReferencedEnvVars(doc), ['BASIC_AUTH_CREDS', 'EDGE_ADMIN_TOKEN', 'JWT_SECRET', 'URL_SIGNING_SECRET']);
});
