...
const { DEFAULT_CONTAINS, parsePathPatterns, hasCatastrophicBacktrackShape, regexesLiteralCode, getAuthGates, validateAuthGates, validateJwksUrl, build, PLACEHOLDER_TOKEN, hasFailOnPermissiveFlag, warnIfPermissive, warnWeakAwsCspNonce, warnUnsupportedAwsResponseDlp, warnSignedUrlReplay, buildGraphqlGuardConfig, warnUnsupportedGraphqlGuard, validateOriginAuth, } = require('./lib/compile-core');
...
test('parsePathPatterns rejects ambiguous regex-like legacy entries', () => {
    assert.throws(() => parsePathPatterns(['(?i)(foo|bar).*']), /Ambiguous path_patterns entry/);
});
...
test('parsePathPatterns rejects invalid regex at build time', () => {
    assert.throws(() => parsePathPatterns({ regex: ['[unterminated'] }), /Invalid regex/);
});
test('parsePathPatterns rejects nested-quantifier ReDoS shapes at build time', () => {
    ...
    assert.throws(() => parsePathPatterns({ regex: ['^(a+)+$'] }), /ReDoS safety check/);
    assert.throws(() => parsePathPatterns({ regex: ['(.*)*'] }), /ReDoS safety check/);
    // With `(?i)` inline flag — must also trigger after the prefix is stripped.
    assert.throws(() => parsePathPatterns({ regex: ['(?i)(x+)+'] }), /ReDoS safety check/);
});
...
test('parsePathPatterns rejects regex-like entries under object-form contains', () => {
    assert.throws(() => parsePathPatterns({ contains: ['(?i)%2f\\.\\./'], regex: [] }), /Ambiguous path_patterns\.contains entry/);
    assert.throws(() => parsePathPatterns({ contains: ['\\.git/'] }), /Ambiguous path_patterns\.contains entry/);
    assert.throws(() => parsePathPatterns({ contains: ['(foo|bar)'] }), /Ambiguous path_patterns\.contains entry/);
});
...
test('getAuthGates throws for missing static_token env without placeholder flag', () => {
    withEnv('EDGE_ADMIN_TOKEN', undefined, () => {
        ...
        assert.throws(() => getAuthGates(policy), /static_token for route "admin" requires env/);
    });
});
...
test('build emits allowedHosts (lowercased) and trustForwardedFor in viewer CFG', () => {
    ...
    try {
        ...
        build(policy, { outDir: tmpDir, rootDir: path.join(__dirname, '..') });
        const code = fs.readFileSync(path.join(tmpDir, 'edge', 'viewer-request.js'), 'utf8');
        assert.ok(code.includes('allowedHosts: ["api.example.com","*.cdn.example.com"]'), 'expected lowercased, trimmed, filtered allowedHosts; got:\n' + code.match(/allowedHosts: .*/)?.[0]);
...
test('build honors explicit allowed_algorithms matching the configured algorithm and filters alg=none', () => {
    ...
    try {
        ...
        build(policy, { outDir: tmpDir, rootDir: path.join(__dirname, '..') });
        const code = fs.readFileSync(path.join(tmpDir, 'edge', 'origin-request.js'), 'utf8');
        assert.ok(code.includes('"allowed_algorithms":["RS256"]'), 'allowed_algorithms should filter "none" and retain only the configured algorithm; got:\n' +
...
test('build filters cross-alg entries from emitted allowed_algorithms even when validateAuthGates is bypassed', () => {
    ...
    try {
        ...
        build(policy, { outDir: tmpDir, rootDir: path.join(__dirname, '..') });
        const code = fs.readFileSync(path.join(tmpDir, 'edge', 'origin-request.js'), 'utf8');
        assert.ok(code.includes('"allowed_algorithms":["RS256"]'), 'emission must fall back to configured algorithm when no entry matches; got:\n' +
            code.match(/"allowed_algorithms":[^,}]+/)?.[0]);
    }
    finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
...
test('warnIfPermissive fails when failOnPermissive is true', () => {
    const captured = [];
    const logger = { error: (msg) => captured.push(msg) };
    const result = warnIfPermissive({ metadata: { risk_level: 'permissive' } }, { logger, failOnPermissive: true });
...
test('validateJwksUrl accepts well-formed public https URLs', () => {
    assert.deepStrictEqual(validateJwksUrl('https://idp.example.com/.well-known/jwks.json').ok, true);
    assert.deepStrictEqual(validateJwksUrl('https://login.microsoftonline.com/common/discovery/v2.0/keys').ok, true);
});
...
test('viewer-request uses fixed-pad constant-time compare (no length short-circuit)', () => {
    ...
    try {
        ...
        assert.ok(!/if\s*\(\s*a\.length\s*!==?\s*b\.length\s*\)\s*return\s+false/.test(vr), 'constantTimeEqual must not short-circuit on length');
    }
    finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
    }
...
test('request.limits.max_header_count defaults to 64 and is clamped to 1..500', () => {
    ...
    try {
        ...
        // Custom
        build({ ...base, request: { ...base.request, limits: { max_header_count: 128 } } }, { outDir: tmpDir, allowPlaceholderToken: true });
        vr = fs.readFileSync(path.join(tmpDir, 'edge', 'viewer-request.js'), 'utf8');
        assert.match(vr, /maxHeaderCount: 128,/);
        // Clamp high
        build({ ...base, request: { ...base.request, limits: { max_header_count: 9999 } } }, { outDir: tmpDir, allowPlaceholderToken: true });
        vr = fs.readFileSync(path.join(tmpDir, 'edge', 'viewer-request.js'), 'utf8');
        assert.match(vr, /maxHeaderCount: 500,/);
        // Clamp low
        build({ ...base, request: { ...base.request, limits: { max_header_count: 0 } } }, { outDir: tmpDir, allowPlaceholderToken: true });
        vr = fs.readFileSync(path.join(tmpDir, 'edge', 'viewer-request.js'), 'utf8');
