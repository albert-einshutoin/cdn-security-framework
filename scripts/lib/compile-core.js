    ...
    for (const route of routes) {
        ...
        if (authType === 'jwt') {
            ...
            if (Array.isArray(gate.allowed_algorithms) && gate.allowed_algorithms.length > 0) {
                const extras = gate.allowed_algorithms.filter((a) => typeof a === 'string' && a !== 'none' && a !== alg);
                if (extras.length > 0) {
                    errors.push(`Route "${name}": auth_gate.allowed_algorithms contains ${JSON.stringify(extras)} ` +
            ...
            if (!resolved && !allowPlaceholderToken) {
                errors.push(`Route "${name}": static_token requires env "${tokenEnv}" at build time. ` +
            ...
            if (!resolved && !allowPlaceholderToken) {
                errors.push(`Route "${name}": basic_auth requires env "${credEnv}" at build time. ` +
                    'Set the env var, or pass --allow-placeholder-token for non-production builds.');
            }
        }
    }
...
function validateOriginAuth(policy, options = {}) {
    ...
    if (!envName) {
        ...
        const v = env[envName];
        if (v === undefined) {
            (strict ? errors : warnings).push(`origin.auth.secret_env "${envName}" is not set in the build environment. Origin will see an empty auth header at runtime unless the env is populated.`);
        }
        else if (v.length === 0) {
...
function warnSignedUrlReplay(policy, options = {}) {
    ...
    for (const route of routes) {
        const gate = route.auth_gate;
        if (!gate || gate.type !== 'signed_url')
            continue;
        if (gate.nonce_param && typeof gate.nonce_param === 'string' && gate.nonce_param.trim())
            continue;
        const match = route.match || {};
        const prefixes = match.path_prefixes || [];
        const writeLike = prefixes.find((p) => SIGNED_URL_WRITE_PATH_HINTS.some((hint) => p.toLowerCase().includes(hint)));
        if (writeLike) {
            warnings.push(`Route "${route.name || 'unnamed'}": signed_url protects ${JSON.stringify(writeLike)} but has no "nonce_param". ` +
...
function warnIfPermissive(policy, options = {}) {
    ...
    const msg = '[WARN] metadata.risk_level is "permissive" — this profile is intentionally loose and NOT recommended for production. ' +
