...
const { parseArgs, hasFlag, loadPolicyWithWarnings, reportPolicyWarnings, reportPolicyLoadError, } = require('./lib/policy-io');
...
if (Array.isArray(waf.rate_limit_rules)) {
    for (const rule of waf.rate_limit_rules) {
        ...
        if (Object.keys(sd).length > 0 && translatedShape === null) {
            parityWarnings.push(`[cloudflare-waf-parity] APPROXIMATE: rate_limit rule "${rule.name}" has scope_down_statement that this compiler does not auto-translate to a Cloudflare expression (shape: ${Object.keys(sd).join(', ')}). Rule degraded to expression: "true" (match-all). Set rule.expression_cloudflare on the policy to provide the exact scope. See docs/cloudflare-waf-parity.md#scope_down_statement-shapes.`);
            sawApproximationOrUnsupported = true;
        }
...
if (managedEntries.length > 0) {
    ...
    for (const name of managedEntries) {
        ...
        if (cfId) {
            managedRules.push({
                description: entry.status === 'equivalent'
