...
- `bin/`, `lib/`, `scripts/`, `parser/`, `validator/`, `emitter/` - Compiled JavaScript and `.d.ts` package artifacts emitted by `npm run build:ts` (via the `prepare` lifecycle script on `npm ci` / `npm install`). Not tracked in git; do not edit these directly.
...
The authoritative implementation source is `src/**/*.ts`, plus runtime templates under `templates/` and policy/docs files. Root-level JavaScript and `.d.ts` files under `bin/`, `lib/`, `scripts/`, `parser/`, `validator/`, and `emitter/` are generated by `npm run build:ts` and are not committed. The `prepare` script runs `build:ts` automatically after `npm ci` or `npm install`, so a fresh clone is runnable without manual compilation.
...
3. Commit only the `src/**/*.ts` change.
