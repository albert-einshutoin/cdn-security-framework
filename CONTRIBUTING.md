...
- `bin/`, `lib/`, `scripts/`, `parser/`, `validator/`, `emitter/`, `contract/`, `openapi/`, `recommendation/`, `reporters/`, `source-analysis/`, `source/` - Compiled JavaScript and `.d.ts` package artifacts emitted by `npm run build:ts` (via the `prepare` lifecycle script on `npm ci` / `npm install`). Not tracked in git; do not edit these directly.
...
The authoritative implementation source is `src/**/*.ts`, plus runtime templates under `templates/` and policy/docs files. Root-level JavaScript and `.d.ts` files under `bin/`, `lib/`, `scripts/`, `parser/`, `validator/`, `emitter/`, `contract/`, `openapi/`, `recommendation/`, `reporters/`, `source-analysis/`, and `source/` are generated by `npm run build:ts` and are not committed. The `prepare` script runs `build:ts` automatically after `npm ci` or `npm install`, so a fresh clone is runnable without manual compilation.

When changing TypeScript source:

1. Edit the matching file under `src/`.
2. Run `npm run build:ts` (or rely on `prepare` after install) to regenerate package artifacts locally.
3. Run `npm run test:generated-boundary` to verify that no compiled package artifact is tracked.
4. Commit only the `src/**/*.ts` change (plus intentional policy, docs, template, or golden files).

Do not edit generated JavaScript or `.d.ts` files directly.
