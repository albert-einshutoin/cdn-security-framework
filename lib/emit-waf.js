...
function toCloudFormationShape(value) {
    if (Array.isArray(value))
        return value.map(toCloudFormationShape);
...
function cfnOverrideAction(action) {
    if (action && action.count)
        return { Count: {} };
    return { None: {} };
}
function cfnVisibilityConfig(config) {
    return {
...
function cfnRules(rules) {
    return (rules || []).map((rule) => {
        ...
        if (rule.action)
            out.Action = cfnAction(rule.action);
        if (rule.override_action)
            out.OverrideAction = cfnOverrideAction(rule.override_action);
        return out;
    });
}
function cfnCustomResponseBodies(blockResponse) {
    if (!blockResponse)
        return undefined;
    return {
        cdn_sec_block: {
...
function buildCloudFormation(policy, options) {
    ...
    if (responseBodies)
        ruleGroup.Properties.CustomResponseBodies = responseBodies;
    ...
    if (waf.managed_rules && waf.managed_rules.length > 0 && outputMode !== 'rule-group') {
        ...
        const webAcl = {
            ...
            Properties: {
                ...
                Scope: scope,
                DefaultAction: { Allow: {} },
                Rules: waf.managed_rules.map((ruleName, idx) => ({
                    Name: `AWS-${ruleName}`,
                    Priority: 10 + idx,
                    OverrideAction: { None: {} },
                    Statement: {
        ...
        if (responseBodies)
            webAcl.Properties.CustomResponseBodies = responseBodies;
