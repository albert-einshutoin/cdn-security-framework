...
function bodyLength(operation) {
    ...
    if (!body)
        return { value: 0, kind: 'exact', reasons: [] };
...
function queryAuthentication(operation) {
    ...
    for (const alternative of operation.auth.alternatives) {
        ...
        for (const scheme of alternative.schemes) {
            if (scheme.kind !== 'api-key' || scheme.location !== 'query')
                continue;
            if (scheme.parameterName) {
                names.add(scheme.parameterName);
                if (!declared.has(scheme.parameterName))
                    alternativeNames.add(scheme.parameterName);
            }
            else
                unnamed += 1;
        }
...
function operationRecommendation(contract, operation, margin) {
    ...
    if (queryCount.value !== null) {
        ...
        if (queryCount.value === null)
            queryCount.reasons.push('query:count-overflow');
    }
    ...
    return {
        ...
        maxQueryParams: numeric(queryCount.value, basis, queryCount.value === null ? 'unknown' : querySurfaceComplete ? 'upper-bound' : 'partial', margin, queryCount.reasons),
        maxQueryLength: numeric(query.value, basis, query.value === null ? 'unknown' : querySurfaceComplete ? 'upper-bound' : 'partial', margin, query.reasons),
        maxUriLength: numeric(uri.value, basis, uri.value === null ? 'unknown' : parametersComplete ? 'upper-bound' : 'partial', margin, uri.reasons),
        maxBodyBytes: numeric(body.value, basis, body.value === null ? 'unknown' : body.kind === 'exact' && bodiesComplete ? 'exact' : 'partial', margin, body.reasons),
