...
const CFG = {
    mode: "enforce", // "enforce" | "monitor" (monitor requires logging; prefer enforce for now)

    // Allowed methods at the edge (use a separate behavior for API if you need PUT/DELETE)
    allowMethods: ["GET", "HEAD", "POST"],

    // Query limits (DoS / cache / log pollution mitigation)
    maxQueryLength: 1024,
    maxQueryParams: 30,

    // Drop query keys to avoid cache-key pollution
    ...
    // Coarse scanner/automation block (full bot handling is WAF)
    uaDenyContains: ["sqlmap", "nikto", "acunetix", "masscan", "python-requests", "zgrab", "nmap"],

    // Coarse path traversal block (full coverage is WAF managed rules)
    blockPathMarks: ["/../", "%2e%2e", "%2f..", "..%2f", "%5c"],

    // Optional: simple token gate for admin paths
    adminGate: {
      enabled: true,
      protectedPrefixes: ["/admin", "/docs", "/swagger"],
      tokenHeaderName: "x-edge-token",
      // NOTE: CloudFront Functions often cannot reference external secrets; inject at build time (or via compiler later)
