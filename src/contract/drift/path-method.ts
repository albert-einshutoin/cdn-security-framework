...
export function comparePathMethodContracts(input: ContractDriftInput): SecurityFindingV1[] {
  ...
  for (const rule of allowed.orderedRules) {
    if (rule.auth.kind === 'none') continue;
    ...
    for (const prefix of policyPaths) {
      ...
      if (rule.auth.exactPath
        && !declaredShapes.has(normalizedPathShape(prefix))
        ...
        findings.push(makeFinding({
          ...
          evidence: [policyEvidence(
            ...
            rule.match.values.includes(prefix)
              ...
              : `${rule.pointer}/auth_gate/exact_path`,
        ...
        const relations = declared.operations.map((operation) => pathRelation(operation, prefix, allowed));
        findings.push(makeFinding({
