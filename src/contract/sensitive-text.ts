export const SENSITIVE_KEY_PATTERN = /(?:authorization|cookie|set[-_]?cookie|api[-_]?key|access[_-]?token|refresh[_-]?token|client[-_]?secret|token|secret|password)/i;
...
export function redactSensitiveText(value: string): string {
  return value
    ...
    .replace(/\b(Basic|Bearer|Negotiate)\s+[^\s,;]+/gi, '$1 [REDACTED]');
}
