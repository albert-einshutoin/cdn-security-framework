...
export function normalizeOpenApiOperations(
  ...
  let authenticationCapability: 'complete' | 'partial' = 'complete';
  ...
  const securitySchemesLocation = locatedChild(
    componentsLocation,
    'securitySchemes',
    components.securitySchemes ?? {},
  );
  const securitySchemes = asObject(securitySchemesLocation);
  if (Object.keys(securitySchemes).length > limits.maxSecuritySchemes) fail(securitySchemesLocation);
  const schemeDefinitions = new Map<string, {
    scheme: Omit<ApiAuthSchemeV1, 'scopes'>;
    evidence: FindingEvidenceV1;
  }>();
  for (const name of Object.keys(securitySchemes).sort(compareText)) {
    const definitionLocation = materialize(locatedChild(
      securitySchemesLocation,
      name,
      securitySchemes[name],
    ));
    const definition = asObject(definitionLocation);
    if (typeof definition.type !== 'string' || !definition.type.trim()) fail(definitionLocation);
    let scheme: Omit<ApiAuthSchemeV1, 'scopes'>;
    if (definition.type === 'http') {
      if (typeof definition.scheme !== 'string' || !definition.scheme.trim()) fail(definitionLocation);
      ...
      if (typeof definition.name !== 'string' || !definition.name.trim()
        ...
        || definition.in === 'path') fail(definitionLocation);
      const location = definition.in as 'header' | 'query' | 'cookie';
      scheme = {
        name,
        kind: 'api-key',
        location,
      ...
      if (names.length === 0) fail(flowsLocation);
      for (const flow of names) {
        ...
        if (Object.values(scopes).some((description) => typeof description !== 'string')) fail(flowLocation);
      }
    ...
    if (scheme.capability === 'unsupported') authenticationCapability = 'partial';
  ...
  const normalizeAuthentication = (
    input: unknown,
    location: LocatedValue | undefined,
  ): { auth: ApiAuthenticationContractV1; exposure: 'public' | 'authenticated' | 'unknown'; evidence: FindingEvidenceV1[] } => {
    if (location === undefined) {
      authenticationCapability = 'partial';
      return { auth: { mode: 'unknown', alternatives: [] }, exposure: 'unknown', evidence: [] };
    }
    if (!Array.isArray(input) || input.length > limits.maxSecuritySchemes) fail(location);
    ...
    const alternatives = input.map((value, index) => {
      ...
      if (names.length > limits.maxSecuritySchemes) fail(requirementLocation);
      const schemes = names.map((name) => {
        ...
        if (!definition || !Array.isArray(scopes)
          || scopes.some((scope) => typeof scope !== 'string' || !scope.trim())) fail(requirementLocation);
        ...
        return { ...definition.scheme, scopes } as ApiAuthSchemeV1;
      });
  ...
  for (const routePath of Object.keys(paths).sort(compareText)) {
    ...
    for (const methodKey of Object.keys(pathItem).filter((key) => METHOD_KEYS.has(key.toLowerCase())).sort(compareText)) {
      ...
      const authentication = normalizeAuthentication(
        operationSecurityLocation === undefined
          ? rootSecurityLocation?.value
          : operationSecurityLocation.value,
        operationSecurityLocation ?? rootSecurityLocation,
      );
      operations.push({
        method: methodKey,
        path: canonicalPath,
