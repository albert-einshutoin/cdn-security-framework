...
export interface TypeScriptProjectFileSystem {
  ...
  readFileBounded(
    filePath: string,
    maxBytes: number,
    expectedIdentity?: Readonly<Pick<fs.Stats, 'dev' | 'ino'>>,
  ): string;
...
function isPathLikeSpecifier(value: string): boolean {
  ...
  return path.isAbsolute(normalized) || /^[a-z]:\//iu.test(normalized)
...
interface ConfigState {
  ...
  readonly snapshotTargets: Map<string, string>;
  totalBytes: number;
  largestFileBytes: number;
}

function bindConfigSnapshot(state: ConfigState, lexicalPath: string, absolute: string, text: string): void {
...
function validateConfigTree(
  ...
  if (state.digests.has(resolved.relative)) {
    ...
    if (contents !== undefined) bindConfigSnapshot(state, lexicalPath, resolved.absolute, contents);
    return;
  }
  ...
  if (compilerOptions !== undefined) {
    ...
    if (compiler.types !== undefined) {
      ...
      for (const value of compiler.types as string[]) {
...
function createParseHost(
  ...
  return {
    ...
    fileExists: (candidate) => validatedConfigs.has(path.resolve(candidate)),
    readFile: (candidate) => validatedConfigs.get(path.resolve(candidate)),
...
async function loadTypeScriptProjectInternal(
  ...
  const read = ts.readConfigFile(
    resolvedConfig.absolute,
    (candidate) => configState.snapshots.get(path.resolve(candidate)),
  );
  if (read.error) {
    throw new TypeScriptProjectLoadError('TS_PROJECT_INVALID_CONFIG', {
  ...
  const preflightProgramRead = (absolute: string, sourceUri?: string): fs.Stats => {
    let stat: fs.Stats;
  ...
  const validateReferences = (text: string, relative: string): void => {
    ...
    )) || preprocessed.typeReferenceDirectives.some(({ fileName }) => (
      path.isAbsolute(fileName.replaceAll('\\', '/')) || /^[a-z]:[\\/]/iu.test(fileName)
    ))) {
    ...
    if (references.some(({ fileName }) => {
      ...
      if (!isPathLikeSpecifier(normalized)) return false;
