...
import { sameFile, writeSafeOutput, type OutputTarget } from './safe-output';
...
function writeOutput(target: OutputTarget, content: string): void {
  try { writeSafeOutput(target, content); }
  catch { throw new ContractDiffCliError('CONTRACT_DIFF_OUTPUT_WRITE_FAILED', 'Report output could not be written safely.'); }
}
