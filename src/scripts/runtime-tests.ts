...
async function runOriginAuthHmacTests() {
  ...
  if (!bodyHashHandler) {
    ...
    const missingBodyResult: any = await bodyHashHandler(buildLambdaEdgeEvent('/origin/upload', {}, '', 'POST'));
    missingBodyOk = !!(missingBodyResult && missingBodyResult.status === '503');
    if (!missingBodyOk) {
      console.error('FAIL: origin auth HMAC should fail closed when POST body is unavailable, got',
