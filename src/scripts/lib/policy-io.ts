 ...
 */
export function defaultOutDir(rootDir: string): string {
...
export function parseCompilerArgs(argv: string[], rootDir: string): CompilerArgs {
 ...
 */
export function loadPolicy(policyPath: string): any {
 ...
 */
export function loadPolicyWithWarnings(policyPath: string): PolicyLoadResult {
  const parsed = parsePolicyFile({ policyPath });
  if (!parsed.ok) {
    const message = parsed.errors.join('; ') || 'failed to parse policy';
    const err: PolicyLoadError = new Error(message);
...
export function reportPolicyWarnings(warnings: string[], policyPath?: string, logger: Pick<Console, 'warn'> = console): void {
