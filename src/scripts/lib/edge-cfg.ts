...
function parsePathPatterns(pathPatterns: unknown) {
  if (pathPatterns === undefined || pathPatterns === null) {
    return { contains: DEFAULT_CONTAINS.slice(), regexSources: [] as string[] };
  }
  ...
  if (typeof pathPatterns === 'object' && pathPatterns !== null) {
    const obj = pathPatterns as { contains?: unknown; regex?: unknown };
...
function buildAnomalyGuardConfig(policy: { request?: { anomaly_guards?: Record<string, unknown> } }) {
...
function findAdminCacheRoute(routes: Array<{ match?: { path_prefixes?: string[] }; auth_gate?: unknown; response?: { cache_control?: string } }>) {
  let adminPathPrefixes: string[] = [];
...
function collectAuthProtectedPrefixes(authGates: Array<{ protectedPrefixes?: string[] }>) {
...
export function buildResponseCfgBase(
  ...
  authGates: Array<{ protectedPrefixes?: string[] }>,
) {
...
function buildJwksCacheCfg(policy: { firewall?: { jwks?: Record<string, unknown> } }) {
  const jwksGlobal = (policy.firewall || {}).jwks || {};
  return {
    staleIfErrorSec: clampNumber(
      jwksGlobal.stale_if_error_sec,
      0,
      JWKS_DEFAULTS.staleMax,
      JWKS_DEFAULTS.staleIfErrorSec,
    ),
    negativeCacheSec: clampNumber(
...
function buildJwtGateConfig(
  gate: Record<string, unknown>,
  base: { name: string; protectedPrefixes: string[]; type: string },
) {
...
function buildSignedUrlGateConfig(
  gate: Record<string, unknown>,
  base: { name: string; protectedPrefixes: string[]; type: string },
) {
  return {
...
export function buildRequestCfgBase(policy: {
  ...
  const normalize = (request.normalize || {}) as Record<string, unknown>;
  ...
  const pathNormalize = (normalize.path || {}) as Record<string, unknown>;
