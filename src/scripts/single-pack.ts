...
export function aggregate(m: Identity, results: Result[], e: ReturnType<typeof expected>, states: string[]): void {
  ...
  for (const row of matrixRows) {
    ...
    assert.deepEqual(r.checks, lower ? ['node-rejection', 'resolution', 'no-side-effects'] : ['package-smoke', 'resolution', 'schemas', 'official-sarif-schema', 'source-aware-cli', 'source-auth-config', 'source-safe-output', ...(row === '24' ? ['source-global-prefix', 'source-controller-options', 'source-uri-version'] : [])]);
    if (row === '24') {
      ...
      assert.deepEqual(r.steps.filter(s => s.command === 'cdn-security-uri-version').map(s => s.expectedExit),
        [0, 0, 0, 0, 0, 1, 2, 2, 2], 'missing installed URI CLI scenarios');
      assert.deepEqual(r.steps.filter(s => s.command === 'source-aware-ci-uri').map(s => s.expectedExit),
        [0, 0, 0, 0], 'missing installed URI CI driver scenarios');
      const uri = r.uriVersionProof;
      assert.ok(uri && Object.keys(uri).sort().join() === [
        ...
        'ciDelivery'].sort().join(), 'missing installed URI proof');
      for (const key of ['inputSha256', 'savedSha256', 'ciRecordSha256'] as const) {
        assert.match(uri[key], /^[a-f0-9]{64}$/);
      }
      for (const key of ['projectDigest', 'configDigest', 'routingDigest',
        'comparisonContractDigest', 'metadataDigest'] as const) {
        assert.match(uri[key], /^sha256:[a-f0-9]{64}$/);
      }
      assert.deepEqual(uri.routes, ['GET /api/v1/users', 'GET /api/v2/users', 'GET /api/v2/users/items']);
      assert.deepEqual(uri.sourceOnly, uri.routes);
      assert.equal(uri.ciDelivery, 'CI_OK');
      const j = r.journey;
...
function consume(directory: string, row: string, output: string): void {
  ...
  let uriVersionProof: import('./package-smoke-tests').UriVersionProof | undefined;
  if (rejected) steps = rejection(consumer);
    ...
    if (row === '24') {
      ...
      const uri = smoke.smokeInstalledUriVersion(consumer,
        (value: unknown) => assert.ok(validate(value), 'installed URI SARIF failed pinned official schema'));
      steps.push(...uri.steps);
      uriVersionProof = uri.proof;
