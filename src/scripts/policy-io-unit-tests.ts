...
function writePolicy(rootDir: string, name: string, body?: string): string {
...
function rmTempRoot(rootDir: string): void {
...
test('hasFlag preserves the old exact boolean flag detection semantics', () => {
  ...
  assert.strictEqual(hasFlag('--allow-placeholder-token' as any, '--allow-placeholder-token'), false);
});
...
test('reportPolicyWarnings includes the policy path when provided', () => {
  const lines: string[] = [];
  const logger = { warn: (...args: unknown[]) => lines.push(args.map(String).join(' ')) };
