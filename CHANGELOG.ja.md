...
- **JWT alg 混同攻撃対策**: `verifyJwtRS256` / `verifyJwtHS256`（AWS）および `verifyJwt`（Cloudflare）は、署名検証を走らせる前に JWT の `header.alg` をゲートごとのホワイトリストと照合するようにした。`alg=none` は常に拒否し、デフォルトではそのゲートに設定された `algorithm` のみを受け付ける。複数アルゴリズムを許容したい場合は `auth_gate.allowed_algorithms: [...]` で明示的にオプトインする。従来は偽造された `alg=none` や、RS256 → HS256 への alg 差し替え（公開鍵を HMAC 秘密鍵として扱わせるクラシックな攻撃）で署名検証を迂回できる余地があった。
