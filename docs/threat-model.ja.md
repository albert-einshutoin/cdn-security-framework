...
**フレームワーク**: Edge の auth_gate（静的トークン）。JWTにはCloudflare Workersまたは独立したviewer-request認証を使用。AWS JWT/署名URLはcache hitでorigin-request検証が省略されるためbuildを拒否。
...
| JWKS ホストの DNS 解決先がクラウドメタデータ、ループバック、RFC1918、リンクローカル IP | Cloudflare は RS256 build で `firewall.jwks.allowed_hosts` を必須化 | — |
| JWKS ホストから内部エンドポイントへの攻撃者制御リダイレクト | 3xx レスポンスを拒否（Workers: `redirect: 'error'`） | — |
...
| 同一パス上でリソース選択用クエリを差し替えて署名 URL を再利用する | 署名自体を除く全クエリパラメータを HMAC 入力に含める | — |
| 1 本の署名 URL が TTL 内で繰り返し使われる | `nonce_param` で HMAC 入力にノンスを束ね、エッジが `X-Signed-URL-Nonce` を転送 | オリジンで単回利用を強制（例: Redis `SET NX`） |
| ノンスを書き換えて他セッションと衝突させる | ノンスは署名対象クエリに含まれ、改ざんで署名が失敗 | — |
| 書き込み系（POST/PUT/DELETE）を長寿命の署名 URL で保護 | `signed_url` が書き込み系プレフィックスに適用され、`nonce_param` 未設定のときはビルド時に警告 | — |

**フレームワーク**: 署名ルール、canonical query binding、ノンス書式（16〜256 文字、URL セーフ unreserved）、オリジン側のパターンは `docs/signed-urls.ja.md` を参照。エッジは署名とノンス束縛を検証するが、単回利用はエッジ関数がステートレスであるためオリジン側との協調が必須。
...
| nonce 伝達手段がないとインライン `<script>` が `'unsafe-inline'` を要求する | Cloudflare は origin fetch 前に nonce を生成して request で渡し CSP に同じ値を使う。AWS は `csp_nonce` を拒否し origin 管理を要求する | — |
...
**フレームワーク**: Cloudflareでは3ウィンドウすべてを実装済み。AWS JWTゲートはcache hitでorigin-request認証が省略されるため生成前に拒否。挙動マトリクスは `docs/auth.ja.md` を参照。
