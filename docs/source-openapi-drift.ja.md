...
- `SC-AUTHZ-001`（warning / high-confidence）: 明示設定した`declaredPrivilegedRoles`と、完全な`authorization` evidenceを持つ高confidenceなSource role metadataが異なる。route名やtag名からprivilegeを推測しない。route identityがpartialならconfidenceをheuristicへ落とす。
