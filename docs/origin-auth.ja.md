...
既定は `include_body_hash: false` です。すべての edge phase が body を安価に読めるわけではないためです。payload まで署名に束ねたい、かつ runtime が body を読める場合だけ `true` にしてください。AWS Lambda@Edge では body が truncate されている場合、または header から payload が送られたと分かるのに CloudFront が body を含めていない場合、部分 body への署名を避けるため fail closed します。正当に空の request は空 SHA-256 digest として署名します。
